# Koşudan alınan Türkçe özet

Bu metin elle PASS yazılmadı. `report` job’u `node ops/verify/write-summary.mjs` ile üretti.

- PR başı: `05a2df05caef7744508c2f50cf0193e8dfb34a17`
- Checkout / rapordaki SHA: `54268a900ca8e7185ae3edc96a46a4f3561f7119` (`pull_request` merge)
- Koşu: https://github.com/OmerOgucu/yemesek-mi-acaba/actions/runs/36218164866
- Olay: `pull_request`. Tetikleyen: `cursor[bot]`
- `main`, `release`, `verify-notify` bu olayda çalışmadı
- `@OmerOgucu` yorumu bu birleşme öncesi DOĞRULANMADI
- `docs/VERIFY-SAMPLE.md` şablondur; runtime’ı NOT_RUN’dur ve bu dosyanın yerine geçmez

## Doğrulama özeti

İncelenen SHA: `54268a900ca8e7185ae3edc96a46a4f3561f7119`
Hedef: PR #1
Olay: pull_request
...
| runtime (Compose, restart, Playwright, yedek) | PASS |
...
- BULGU id=web-container durum=ÇÖZÜLDÜ sonuç=PASS
...
- BULGU id=unhealthy-deploy durum=ÇÖZÜLDÜ sonuç=PASS
  - Başlık: Sağlıksız deploy veya rollback başarı yazmaz
  - Yer: ops/ci/runtime.sh:148, ops/ci/runtime.sh:138
...
- BULGU id=restore-roundtrip durum=ÇÖZÜLDÜ sonuç=PASS
  - Başlık: Uzak yedek disposable veritabanına döner
  - Yer: ops/ci/runtime.sh:126
...
- BULGU id=clean-host durum=ÇÖZÜLDÜ sonuç=PASS
  - Başlık: Temiz Linux, Compose ve tarayıcı akışı
  - Yer: ops/ci/runtime.sh:40, ops/e2e/user.spec.ts:9, ops/e2e/admin.spec.ts:15
